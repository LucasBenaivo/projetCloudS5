            ...
            if (quantityToSell > maxQuantity) {
             ...
             // Récupérer le graphique Highcharts
                ...
                if (visibleSeries) {
                    ...
                    xhr.open('POST', '<%= request.getContextPath() %>/admin/AdminTransactionCryptoServlet?action=vendre&idCrypto=' + idCrypto + '&quantite=' + quantityToSell + '&prixUnitaire=' + lastPrice, true);
...
function handleBuy() {
    ...
    if (visibleSeries) {
        ...
        if (solde >= montant) {
            ...
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 
            '<%= request.getContextPath() %>/admin/AdminTransactionCryptoServlet?action=acheter&idCrypto='+seriesName+'&quantite='+quantityToBuy+'&prixUnitaire='+lastPrice, true); // Remplacez 'votreServlet' par le chemin de votre servlet
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
